...
const SEL = process.env.SEL || '.lb__quadro';
...
const r = await p.evaluate((SEL) => {
  ...
  const saida = [];
  for (const cel of document.querySelectorAll(SEL)) {
    const campo = rgb(getComputedStyle(cel).backgroundColor);
    const sv = cel.querySelector('.dg');
    /* Nem todo contêiner tem diagrama: na apresentação a maioria das cenas é
       tipo puro. Sem esta guarda o arnês morre na primeira cena sem SVG. */
    if (!sv) continue;
