...
/* Nome próprio, e não `esc`. Todos os módulos entram no MESMO escopo quando a
   página é embutida, e três `const esc` viram `Identifier 'esc' has already been
   declared` — erro de sintaxe, que mata o script inteiro antes da primeira
   linha rodar. Foi o que aconteceu com a apresentação publicada. */
const escDg = (s) => String(s)
...
const rot = (x, y, txt, o = {}) => {
  ...
  return `<text x="${x}" y="${y}" class="dg__rot ${cls}" text-anchor="${anc}" font-size="${tam}"`
       + ` fill="currentColor" fill-opacity="${op}" ${em(b, ate)}${g}>${escDg(txt)}</text>`;
};
...
const txt = (x, y, s, o = {}) => {
  ...
  return `<text x="${x}" y="${y}" class="dg__txt ${cls}" text-anchor="${anc}" font-size="${tam}"`
       + ` font-weight="${peso}" fill="currentColor" fill-opacity="${op}" ${em(b, ate)}>${escDg(s)}</text>`;
};
...
const svg = (id, w, h, rotulo, corpo) =>
  `<svg class="dg" viewBox="0 0 ${w} ${h}" role="img" aria-label="${escDg(rotulo)}"`
