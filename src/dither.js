...
/* Resolve a forma de uma config, com suporte a `morf: {de, para, optsDe,
   optsPara}` — que existe para o morf continuar sendo DADO (serializável em
   JSON, declarável num data-attribute) em vez de exigir uma função escrita à
   mão em cada peça. */
function campoDe(cfg, t) {
  if (cfg.morf) {
...
export function anima(canvas, de, para, duration = 900, easing = (t) => 1 - Math.pow(1 - t, 3)) {
  ...
  if (parado()) {
    parar(canvas);
    const f = campoDe(fim, 1);
    return render(canvas, f ? { ...fim, shape: f } : fim);
  }
  laco(canvas, duration, (t) => {
    const e = easing(t);
    const cfg = interp(de, fim, e);
    const f = campoDe(fim, e);
    render(canvas, f ? { ...cfg, shape: f } : cfg);
  });
}
